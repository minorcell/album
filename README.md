...
- 📷 **图片上传**：成员可批量上传 JPG/PNG，自动生成缩略图并存储至火山对象存储（TOS），支持原始文件名保留
...
# 火山对象存储（TOS）配置，示例为华北 2（北京）
TOS_ACCESS_KEY_ID="your_ak"
TOS_SECRET_ACCESS_KEY="your_sk"
TOS_REGION="cn-beijing"
TOS_ENDPOINT="tos-cn-beijing.volces.com"
TOS_BUCKET="your_bucket"

# 对外访问域名（可为官方域名或自定义域名）
NEXT_PUBLIC_TOS_BASE_URL="https://your_bucket.tos-cn-beijing.volces.com"

# 可选：自定义对象前缀
TOS_UPLOAD_PREFIX="uploads/"
TOS_THUMBNAIL_PREFIX="uploads/thumbnails/"
```
...
5. 将原图上传至火山对象存储桶内的 `uploads/`
6. 将缩略图上传至 `uploads/thumbnails/`
...
本项目已接入火山对象存储（TOS）。请在 `.env` 中填写 AK/SK、Region、Endpoint、Bucket 以及 `NEXT_PUBLIC_TOS_BASE_URL`，并确保桶中存在 `uploads/` 与 `uploads/thumbnails/` 前缀。
...
# 备份对象存储
# 可使用 tosutil 下载备份或在 TOS 控制台配置生命周期/镜像策略
```
